...
function ajaxForSearch(url, filtro, limit=6, offset=0,orderby="p.id_partido") {
    filtro = filtro || [];
    ajaxPromise(url, 'POST', 'JSON', { filter: JSON.stringify(filtro),'limit':limit,'offset':offset,'orderby':orderby })
        .then(function(data) {
...
function print_filters() {
    ...
    .then(function(data) {
        ...
        let html = '<div class="div-filters">' +
                    ...
                    '<option value = "fecha_partido ASC">Fecha ⬆</option>' +
                    '<option value = "fecha_partido DESC">Fecha ⬇</option>' +
                    '<option value = "visitas ASC">Menos visitados</option>' + 
                    '<option value = "visitas DESC">Mas visitados</option>' +
                    '</select>' +
                    '<input type="button" value="ORDENAR" id="order-btn" class="order-btn"/>' +
                    '</div>';
...
function highlightFilters() {
    ...
    filtros.forEach(function(item) {
        const columna = item[0].replace(/^\w+\./, ''); //pijada
        const valor   = item[1];
...
function applyFilters() {
    ...
    updateFilterBadge();
    const orderby = $('#orderby').val().trim() || 'p.id_partido';
    ajaxForSearch('module/shop/controller/controller_shop.php?op=all_event', filtro, 6, 0, orderby);
    pagination();
}
...
function pagination() {
    const filtro = JSON.parse(localStorage.getItem('filter')) || [];
    const url = 'module/shop/controller/controller_shop.php?op=all_event_count';
    ...
    ajaxPromise(url, 'POST', 'JSON', { filter: JSON.stringify(filtro) })
        .then(function(data) {
            ...
            if (total_prod > 6) {
                total_pages = Math.ceil(total_prod / 6);
                $('#pagination').show();
            } else {
                $('#pagination').hide();
            }
            ...
            $('#pagination').bootpag({
                ...
                const filtroActual = JSON.parse(localStorage.getItem('filter')) || [];
                const orderbyActual = $('#orderby').val().trim() || 'p.id_partido';
            ajaxForSearch('module/shop/controller/controller_shop.php?op=all_event', filtroActual, 6, offset, orderbyActual);
            });
        });
}

function events_related(loaded, idPart, local, visitante, total_items) {
    var items = 4;

    ajaxPromise("module/shop/controller/controller_shop.php?op=events_related", 'POST', 'JSON', {
        'idPart':    idPart,
        'local':     local,
        'visitante': visitante,
        'loaded':    loaded,
    ...
    .then(function(data) {
        ...
        function buildCard(p) {
            ...
            var $card = $('<div></div>').attr({ 'id': 'related-' + p.id_partido, 'class': 'partido-card' })
                .html(`
                    <div class="swiper partido-card-swiper" id="swiper-related-${p.id_partido}">
                        <div class="swiper-wrapper">${slidesHTML}</div>
                        <div class="swiper-button-prev"></div>
                        <div class="swiper-button-next"></div>
                    </div>
            ...
            new Swiper(`#swiper-related-${p.id_partido}`, {
                speed: 500,
                loop: p.imgs_partido && p.imgs_partido.length > 1,
                slidesPerView: 1,
                navigation: {
                    prevEl: `#swiper-related-${p.id_partido} .swiper-button-prev`,
                    nextEl: `#swiper-related-${p.id_partido} .swiper-button-next`,
                }
            });
            ...
            if (loaded == 0) {
        $('<div></div>').attr({ 'id': 'title_content_events', 'class': 'title_content' })
            .appendTo('.results')
            .html(`
                <h2 class="cat">Eventos relacionados</h2>
                <div class="related-grid" id="related-grid"></div>
                <div id="more_event__button" class="more_event__button">
                    <button class="load_more_button">VER MÁS</button>
                </div>
            `);

        for (var row in data) {
            if (data[row].id_partido != undefined) {
                buildCard(data[row]).appendTo('#related-grid');
            }
        }
    }
    ...
    if (loaded >= 3) {
        ...
        var total_restante = total_items - 3;
        if (total_restante <= loaded) {
            $('#more_event__button').empty();
        } else {
            $('#more_event__button').html('<button class="load_more_button">VER MÁS</button>');
        }
    }
    }).catch(function() {
...
function more_events_related(idPart, local, visitante) {
    var items = 0;

    ajaxPromise('module/shop/controller/controller_shop.php?op=count_events_related', 'POST', 'JSON', {
        'idPart':    idPart,
        'local':     local,
        'visitante': visitante
    })
    .then(function(data) {
        ...
        events_related(0, idPart, local, visitante, total_items);

        $(document).on("click", '.load_more_button', function() {
            items = items + 3;
            $('#more_event__button').empty();
            events_related(items, idPart, local, visitante, total_items);
        });
