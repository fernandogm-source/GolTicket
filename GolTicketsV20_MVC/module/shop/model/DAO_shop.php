<?php
$path = $_SERVER['DOCUMENT_ROOT'] . '/GolTicketsV20_MVC/';
include($path . "/model/connect.php");

class DAOShop {

    function select_all_event_count($filter = []) {
        $sql = "SELECT COUNT(DISTINCT p.id_partido) AS contador
                FROM partido p
    ...
    function select_all_event($filter = [],$limit = " ",$offset = " ") {
        $sql = "SELECT p.id_partido, p.nombre_partido, p.fecha_partido, p.precio,p.lat, p.lng,
        ...
        $sql .= " GROUP BY p.id_partido LIMIT $limit OFFSET $offset";
    ...
    $sql = "SELECT p.id_partido, p.nombre_partido, p.fecha_partido, p.precio, p.lat, p.lng,
                p.id_equipolocal, p.id_equipovisitante,
    ...
    function count_more_events_related($local, $visitante) {
    $sql = "SELECT COUNT(DISTINCT p.id_partido) - 1 AS n_prod
            FROM partido p
            WHERE p.id_equipolocal = :local 
               OR p.id_equipovisitante = :local 
               OR p.id_equipolocal = :visitante 
               OR p.id_equipovisitante = :visitante";

    $conexion = connect::con();
    $stmt = $conexion->prepare($sql);
    $stmt->execute([':local' => $local, ':visitante' => $visitante]);
    $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
    connect::close($conexion);
    ...
    function select_events_related($idPart, $local, $visitante, $loaded, $items) {
    $loaded = (int) $loaded;
    $items  = (int) $items;

    $sql = "SELECT p.id_partido, p.nombre_partido, p.fecha_partido, p.precio,
                el.nombre_equipo AS nombre_local, ev.nombre_equipo AS nombre_visitante,
                c.nombre_campo, c.img_campo, ci.nombre_ciudad, co.nombre_competicion,
                GROUP_CONCAT(DISTINCT i.img SEPARATOR ':') AS imgs_partido
            FROM partido p
            JOIN equipo el      ON p.id_equipolocal     = el.id_equipo
            JOIN equipo ev      ON p.id_equipovisitante = ev.id_equipo
    ...
    // Igual que en select_all_event: combinar img_campo + imgs_partido
    foreach ($rows as &$row) {
        $imgs_partido = $row['imgs_partido'] ? explode(':', $row['imgs_partido']) : [];
        $imgs_campo   = $row['img_campo']    ? [$row['img_campo']]               : [];
        $row['imgs_partido'] = array_merge($imgs_campo, $imgs_partido);
    }
