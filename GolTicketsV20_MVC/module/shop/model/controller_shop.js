...
function ajaxForSearch(url, filtro, limit=6, offset=0) {
    filtro = filtro || [];
    ajaxPromise(url, 'POST', 'JSON', { filter: JSON.stringify(filtro),'limit':limit,'offset':offset })
        .then(function(data) {
    ...
    ajaxPromise(url, 'POST', 'JSON', { filter: JSON.stringify(filtro) })
        .then(function(data) {
            ...
            $('#pagination').bootpag({
                ...
                ajaxForSearch('module/shop/controller/controller_shop.php?op=all_event', filtroActual, 6, offset);
            });
        });
}
