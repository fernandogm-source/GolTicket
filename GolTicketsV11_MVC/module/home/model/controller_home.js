    ...
    .then(function(data) {
        for (let row = 0; row < data.length; row++) {
            ...
            $('<div></div>')
                ...
                .html(
                    '<div class="partido-slide-inner" style="background-image: url(\'' + data[row].img_campo + '\')">' +
...
function loadCities() {
    ...
    .then(function(data) {
        for (let row = 0; row < data.length; row++) {
            const name = data[row].nombre_ciudad;
            const icon = data[row].img_ciudad;
            ...
            $('<div></div>')
                ...
                .html(
                    '<div class="category-card city-card" style="background-image:url(\'' + icon + '\');background-size:cover;background-position:center;">' +
...
function loadTeams() {
    ...
    .then(function(data) {
        for (let row = 0; row < data.length; row++) {
            const name = data[row].nombre_equipo;
            const icon = data[row].img_equipo;
